...
current_dir = os.getcwd() #用于返回当前工作目录。
sess = tf.InteractiveSession() 

def weight_variable(shape, name):
    initial = tf.truncated_normal(shape, stddev = 0.1)#shape: 一维的张量，也是输出的张量 stddev: 正态分布的标准差
...
saver.restore(sess, os.path.join(current_dir, "C:\\Users\\sue\\.spyder-py3\\MNIST_data\\model.ckpt.index"))

def pca(X, n_components):
    pca = PCA(n_components = n_components)
    pca.fit(X)
    return pca.transform(X)
#PCA 就是通过寻找高维空间中，数据变化最快（方差最大）的方向，对空间的基进行变换，
